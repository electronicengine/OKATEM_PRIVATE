...
int SerialLib::readUart(unsigned char *RecievedData)
{
	...
	unsigned char rx_package[TRANSMISSION_SIZE];
	...
	if (gFileDescriptor != -1)
	{
		...
		if(data_count == TRANSMISSION_SIZE)
		{
			memcpy(RecievedData, &rx_package[0], TRANSMISSION_SIZE);
			data_count = 0;
			return SUCCESS;
		}
		else
...
int SerialLib::writeUart(unsigned char* TransmittedData)
{
	...
	if (gFileDescriptor != -1)
	{
		for(int i=0; i<(TRANSMISSION_SIZE/TRANSMISSION_SIZE); i++)
		{
...
void SerialLib::checkData()
{
	...
	int ret;
	unsigned char header[TRANSMISSION_HEADER_SIZE];
	...
	if (gFileDescriptor != -1)
	{
		while(1)
		{
			...
			if(ret == SUCCESS)
			{
				memcpy(&header[0], &rx_buffer[0], TRANSMISSION_HEADER_SIZE);
				if(strncmp((char*)header, "senv", TRANSMISSION_HEADER_SIZE) == 0)
				{
...
//	rawData = Mat(1, Size, CV_8UC1, longbuf);
...
int SerialLib::sendVideoStream(unsigned char* Data, uint32_t Size)
{
	...
	writeUart(&data_header[0]); // file_descriptor is sending
